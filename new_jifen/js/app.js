...
var weddingSignComponent = {
  ...
  template: '\
    <section class="WX_V3_daily_sign">\
      <div v-bind:style="{\'background-image\':\'url(\'+data.banner+\')\'}">\
        <i class="iconfont icon-guanbi1" v-on:click.prevent="data.isShow=false"></i>\
        <header>签到成功</header>\
        <div class="day">{{data.plus[data.day-1]}}</div>\
        <div class="tips">已连续签到<br>连续签到3天有惊喜哦！</div>\
        ...
        <div class="progress">\
          <table>\
            ...
            <td v-bind:class="{active:data.day==3}">+{{data.plus[2]}}<i class="iconfont icon-huangguan"></i><small>第三天</small></td>\
          </table>\
        </div>\
        <footer>\
  ...
  methods: {
    ...
    signFn: function() {
      var self = this;
      $.post('/' + this.$parent.domain + '/cpv2/my/checkIn.html', function(data) {
        if (data.status == false) {
          WXalert(data.message);
        } else {
          self.data.points = data['params']['insertdata']['left_points'];
...
var selectRolesComponent = {
  ...
  template: '\
    <section v-bind:class="{popup:data.isPopup}" class="WX_V3_select_roles_form">\
      <form>\
        <i v-if="data.isPopup" class="iconfont icon-guanbi1" v-on:click.prevent="data.isShow=false"></i>\
        <div>\
          <header>您的角色</header>\
          <table>\
            <td><input v-model="data.sex" type="radio" value="women" id="women"><label for="women"><img v-bind:src="data.women[1]">{{data.women[0]}}<i class="iconfont icon-xuanzhong"></i></label></td>\
            <td>或</td>\
            <td><input v-model="data.sex" type="radio" value="men" id="men"><label for="men"><img v-bind:src="data.men[1]">{{data.men[0]}}<i class="iconfont icon-xuanzhong"></i></label></td>\
          </table>\
        </div>\
        <div>\
          <input v-model="data.name" type="text" placeholder="请输入您的称呼">\
          <input v-model="data.mobile" type="tel" placeholder="请输入您的手机">\
          <input v-model="data.birthday1" v-if="data.birthdayShow" type="date" placeholder="请输入您的生日" required>\
        </div>\
        <div v-if="data.tips!=\'\'" class="tips"><i class="i_horn"></i><a>{{data.tips}}</a></div>\
        <button v-on:click.prevent="rolesFn">确定</button>\
      </form>\
    </section>',
  mounted: function(){ //检查是否会员
    var self = this;
    bus.$on('rolesEvent',function(url, is_sign){
      console.log("self", self.data)
      self.data.isShow = self.data.need_show_roles;
      self.data.url = url;
      self.data.is_sign = is_sign;
      
      //不显示会员框并且是签到
      if (self.data.isShow == false && is_sign == true) {
        bus.$emit('signEvent',self.data);
      } else {
        if (self.data.isShow == false && url != false) {
  ...
  methods: {
    //检查会员弹框
    rolesFn: function() { //选择角色
      var self = this;
      if (!/^(13[0-9]|15[0-9]|18[0-9]|14[0-9]|17[0-9])\d{8}$/.test(self.data.mobile)) {
        return WXalert('请输入正确的手机号码');
      }

      $.post('/' + this.$parent.domain + '/cpv2/my/join.html', {
        name: self.data.name,
        mobile: self.data.mobile,
        gender: self.data.sex == 'women' ? 2 : 1,
        birthday1: self.data.birthday1
      }, function(data) {
        if (data.status == false) {
          WXalert(data.message);
        } else {
          self.data.isShow = false;
          self.data.need_show_roles = false;
          //签到
          if (self.data.is_sign) {
            bus.$emit('signEvent',self.data);

          } else {
            if (self.data.url != false) {
              WXalert('保存信息成功！', function() {
                setTimeout(function() {
...
var weddingDateComponent = {
  ...
  template: '\
      <section class="WX_V3_wedding_date">\
        <div>\
          <i class="iconfont icon-guanbi1" v-on:click="weddingFn"></i>\
