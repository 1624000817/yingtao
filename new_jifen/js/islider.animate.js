    ...
    iSlider && iSlider.extend(iSlider._animateFuncs, {
        ...
        depth: (function () {
            ...
            function depth(dom, axis, scale, i, offset, direct) {
                var zoomScale = (4 - Math.abs(i - 1)) * 0.24;
                var outer = dom.parentElement;
                iSlider.setStyle(outer, 'perspective', scale * 4);
                dom.style.zIndex = i === 1 ? 1 : 0;
                iSlider.setStyle(dom, 'transform', 'scale(' + zoomScale + ') translateZ(0) translate' + axis + '(' + (offset + 1.2 * scale * (i - 1)) + 'px)');
            }
        ...
        flow: (function () {
            function flow(dom, axis, scale, i, offset, direct) {
                ...
                iSlider.setStyle(dom, 'transform', 'scale(1, 1) translateZ(' + (offsetRatio * 150 - 150) * Math.abs(i - 1) + 'px)' + 'translate' + axis + '(' + (offset + scale * (i - 1)) + 'px)' + 'rotate' + rotateDirect + '(' + directAmend * (30 - offsetRatio * 30) * (1 - i) + 'deg)');
            }
